namespace IEnumerableExtensions
{
    ...
    public static class IEnumerableExtends
    {
        ...
        {
            return input.Aggregate<T, dynamic>(0, (current, item) => current + item);
        ...
        {
            return input.Aggregate<T, dynamic>(1, (current, item) => current * item);
        ...
        public static T Min<T>(this IEnumerable<T> input) where T : IComparable
        {
            ...
            foreach (var item in input.Where(item => item < min))
            {
                min = item;
            }
        ...
        public static T Max<T>(this IEnumerable<T> input) where T : IComparable
        {
            ...
            foreach (var item in input.Where(item => item > max))
            {
                max = item;
            }
